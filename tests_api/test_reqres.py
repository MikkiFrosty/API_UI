...
def test_get_single_user(api_client):
    r = api_client.get("/users/2", verify=False, headers=headers)
    assert r.status_code == 200
    validate(r.json(), single_user)

def test_get_single_user_404(api_client):
    r = api_client.get("/users/23", verify=False, headers=headers)
    assert r.status_code == 404
    assert r.json() == {}

def test_post_create_user_schema_from_file(api_client):
    payload = {"name": "morpheus", "job": "leader"}
    r = api_client.post("/users", verify=False, json_body=payload, headers=headers)
...
def test_post_register_missing_password(api_client):
    ...
    r = api_client.post("/register", verify=False, json_body=payload, headers=headers)
    assert r.status_code == 400
    validate(r.json(), error_schema)

def test_put_update_user(api_client):
    payload = {"name": "neo", "job": "the one"}
    r = api_client.put("/users/2", verify=False, json_body=payload, headers=headers)
    assert r.status_code == 200
    validate(r.json(), update_user)

def test_delete_user(api_client):
    r = api_client.delete("/users/2", verify=False, headers=headers)
